...
public class UpdateFeedbackOrderAction implements Action {
	...
	public ApiResponse execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (jsonPayload == null) {
			return new ApiResponse(false, "Missing reorder data.", null);
		}
		...
		try (Connection conn = dbManager.getConnection()) {
			...
			try {
				...
				adminLogService.log(adminUser.getUsername(), "UPDATE_FEEDBACK_ORDER", "Feedback ID " + submissionId
						+ " status auf '" + newStatus + "' gesetzt und Liste neu sortiert.");
				return new ApiResponse(true, "Feedback-Status und Sortierung aktualisiert.", null);
			...
			return new ApiResponse(false, "Serverfehler beim Verarbeiten der Anfrage: " + e.getMessage(), null);
		}
	}
}