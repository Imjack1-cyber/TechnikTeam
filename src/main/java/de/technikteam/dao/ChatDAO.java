...
public class ChatDAO {
	...
	public List<ChatMessage> getMessagesForConversation(int conversationId, int limit, int offset) {
		...
		return jdbcTemplate.query(sql, (rs, rowNum) -> {
			...
			msg.setChatColor(rs.getString("chat_color"));
			msg.setEdited(rs.getBoolean("edited"));
			msg.setDeleted(rs.getBoolean("is_deleted"));
			if (rs.getTimestamp("deleted_at") != null) {
				msg.setDeletedAt(rs.getTimestamp("deleted_at").toLocalDateTime());
			}
			msg.setDeletedByUserId(rs.getObject("deleted_by_user_id", Integer.class));
			msg.setDeletedByUsername(rs.getString("deleted_by_username"));
	...
	public void markMessageAsRead(long messageId, int userId) {
		String sql = "INSERT IGNORE INTO chat_message_read_status (message_id, user_id) VALUES (?, ?)";
	...
	public boolean updateMessagesStatusToRead(List<Long> messageIds, int conversationId, int readerId) {
		...
		String ids = messageIds.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(","));
		String sql = String.format(
				"UPDATE chat_messages SET status = 'READ' WHERE id IN (%s) AND conversation_id = ? AND sender_id != ? AND status != 'READ'",
				ids);
		return jdbcTemplate.update(sql, conversationId, readerId) > 0;
	}

	public boolean updateMessage(long messageId, int userId, String newText) {
		String sql = "UPDATE chat_messages SET message_text = ?, edited = TRUE WHERE id = ? AND sender_id = ? AND is_deleted = FALSE";
	...
	public boolean deleteMessage(long messageId, int deletersUserId, boolean isAdmin) {
		String sql = isAdmin
				? "UPDATE chat_messages SET is_deleted = TRUE, deleted_by_user_id = ?, deleted_at = NOW() WHERE id = ?"
				: "UPDATE chat_messages SET is_deleted = TRUE, deleted_by_user_id = ?, deleted_at = NOW() WHERE id = ? AND sender_id = ?";
