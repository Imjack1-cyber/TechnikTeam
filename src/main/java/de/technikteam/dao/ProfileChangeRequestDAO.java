...
public class ProfileChangeRequestDAO {
	...
	public ProfileChangeRequest getRequestById(int id) {
		String sql = "SELECT pcr.*, u.username as username, a.username as admin_username "
				+ "FROM profile_change_requests pcr " + "JOIN users u ON pcr.user_id = u.id "
				+ "LEFT JOIN users a ON pcr.reviewed_by_admin_id = a.id " + "WHERE pcr.id = ?";
		try (Connection conn = DatabaseManager.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			pstmt.setInt(1, id);
	...
	public List<ProfileChangeRequest> getPendingRequests() {
		...
		String sql = "SELECT pcr.*, u.username as username FROM profile_change_requests pcr "
				+ "JOIN users u ON pcr.user_id = u.id " + "WHERE pcr.status = 'PENDING' ORDER BY pcr.requested_at ASC";
	...
	private ProfileChangeRequest mapResultSetToRequest(ResultSet rs) throws SQLException {
		...
		// Joined columns
		if (rs.getMetaData().getColumnCount() > 7) { // Simple check if joins were present
