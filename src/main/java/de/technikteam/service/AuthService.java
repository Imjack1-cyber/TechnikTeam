...
public class AuthService {
	...
	public String generateToken(User user) {
		...
		Instant expiry = now.plus(8, ChronoUnit.HOURS); // Token is valid for 8 hours
	...
	/**
	 * Validates a JWT and retrieves the corresponding user from the database.
	 * 
	 * @param token The JWT string from the Authorization header.
	 * @return An Optional containing the User if the token is valid and the user
	...
	public User validateTokenAndGetUser(String token) {
		try {
			...
			// Fetch a fresh user object from the DB to ensure permissions are up-to-date
