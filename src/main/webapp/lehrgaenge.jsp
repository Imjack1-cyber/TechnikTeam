...
<div class="card">
    <table class="styled-table">
        <thead><tr><th>Name</th><th>Typ</th><th>Datum & Uhrzeit</th><th>Leitung</th><th>Deine Teilnahme</th></tr></thead>
        <tbody id="course-table-body">
            <c:choose>
                ...
                <c:otherwise>
                    <tr><td colspan="5" style="text-align: center;">Aktuell stehen keine Lehrgänge an.</td></tr>
                </c:otherwise>
            </c:choose>
        </tbody>
    </table>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnOpen.addEventListener('click', () => {
        ...
        rows.forEach(row => {
            row.style.display = row.dataset.attended === 'false' ? '' : 'none';
        });
    });
});
</script>
<style>.filter-controls .btn-small.active { background-color: var(--primary-color); color: #fff; }</style>
