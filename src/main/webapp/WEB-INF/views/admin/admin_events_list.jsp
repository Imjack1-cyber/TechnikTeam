	...
	<table class="data-table sortable-table searchable-table">
		...
		<tbody>
			...
			<c:forEach var="event" items="${eventList}">
				<tr>
					<td><a
						href="${pageContext.request.contextPath}/events/details?id=${event.id}">${event.name}</a></td>
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const openAssignModal = async (btn) => {
		...
		try {
			const response = await fetch(`${contextPath}/WEB-INF/views/WEB-INF/views/admin/admin_events_list.jsp?action=getAssignmentData&id=${eventId}`);
	...
	const addAttachmentRow = (id, filename, filepath) => {
        ...
        removeBtn.onclick = () => {
            showConfirmationModal(`Anhang '${filename}' wirklich löschen?`, async () => {
                try {
                    const response = await fetch(`${contextPath}/WEB-INF/views/WEB-INF/views/admin/admin_events_list.jsp`, { method: 'POST', body: new URLSearchParams({ action: 'deleteAttachment', id: id}) });
	...
	document.querySelectorAll('.edit-event-btn').forEach(btn => {
		btn.addEventListener('click', async () => {
			const eventId = btn.dataset.eventId;
			try {
				const response = await fetch(`${contextPath}/WEB-INF/views/WEB-INF/views/admin/admin_events_list.jsp?action=getEventData&id=${eventId}`);
