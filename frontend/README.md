...
### Session Management
*   User sessions are managed by a JSON Web Token (JWT) that is stored securely on the device.
*   **Web Sessions:** For security in a browser environment, web session tokens have a lifetime of **8 hours**.
*   **Mobile App Sessions:** To provide a better user experience on native apps, mobile session tokens have an extended lifetime of **336 hours (14 days)**. The app will remember your session even after being closed and reopened.
