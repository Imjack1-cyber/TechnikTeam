            ...
            case 'Details':
                return (
                     <View>
                        ...
                        <TextInput style={styles.input} value={formData.title} onChangeText={val => setFormData({...formData, title: val})} placeholder={isSchedulingType ? "z.B. Sommertreffen" : "z.B. Welches Essen wollt ihr?"} />
                        ...
                        <>
                            ...
                            {optionTexts.map((option, index) => (
                                <View key={index} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.sm }}>
                                    <TextInput style={[styles.input, { flex: 1, marginBottom: 0 }]} value={option} onChangeText={(text) => handleOptionChange(text, index)} placeholder={`Option ${index + 1}`} />
                     ...
                     return (
                        <>
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md, backgroundColor: colors.surface, padding: 8, borderRadius: 8, borderWidth: 1, borderColor: colors.border }}>
                              ...
                              <Switch value={options.allowGuests} onValueChange={val => setOptions(prev => ({ ...prev, allowGuests: val }))} />
                            </View>
                            ...
                            {options.allowGuests && (
                              <>
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md, backgroundColor: colors.surface, padding: 8, borderRadius: 8, borderWidth: 1, borderColor: colors.border }}>
                                  ...
                                  <Switch value={options.requireVerificationCode} onValueChange={val => setOptions(prev => ({ ...prev, requireVerificationCode: val }))} />
                                </View>
                                {options.requireVerificationCode && (
                                  <View>
                                    <Text style={styles.label}>Code</Text>
                                    <TextInput style={styles.input} value={verificationCode} onChangeText={setVerificationCode} placeholder="z.B. Team123" />
                                  </View>
                                )}
                              </>
                            )}
